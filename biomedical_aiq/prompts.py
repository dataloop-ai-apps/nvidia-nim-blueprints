# Prompt templates adapted from NVIDIA Biomedical AI-Q Research Agent Blueprint
# Original: https://github.com/NVIDIA-AI-Blueprints/biomedical-aiq-research-agent
...
2. Ensure the follow-up question is self-contained and includes necessary context for web search.
3. If both of the ingredients are found in the current knowledge base, return the target protein and recent small molecule therapy. If the recent small molecule therapy is a combination of multiple molecules, pick only one molecule. Make sure you return a valid molecule name, and not a branded name for therapy that may contain multiple molecules. Format your response as a JSON object with the following keys:
...
- query: Write a specific follow up question to identify the missing ingredient
- rationale: Describe what information is missing or needs clarification
...
COMBINE_VS_INTO_REPORT = """Given the intended report structure and existing report draft below, add one additional section exactly titled "Running Virtual Screening for Novel Small Molecule Therapies" into the intended report structure, before the Conclusions section.
