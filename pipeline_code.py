...
# # response = llm.call(messages=[{"role": "user", "content": "What's a good name for a dog?"}])
...
# project_url = "https://github.com/dataloop-ai-apps/active-learning"
...
create_documentation_plan = Task(
    config=tasks_config['create_documentation_plan'], agent=documentation_planner, output_pydantic=DocPlan
)

planning_crew = Crew(
    agents=[code_explorer, documentation_planner], tasks=[analyze_codebase, create_documentation_plan], verbose=False
)
...
overview_writer = Agent(
    ...
    tools=[
        ...
        WebsiteSearchTool(
            ...
            config=dict(embedder=dict(provider="nvidia", config=dict(model="nvidia/nv-embedqa-e5-v5"))),
        ),
    ],
)
...
documentation_reviewer = Agent(
    ...
    tools=[DirectoryReadTool(directory="docs/", name="Check existing documentation folder"), FileReadTool()],
)

draft_documentation = Task(config=tasks_config['draft_documentation'], agent=overview_writer)
...
# saved to `docs` dir in project root
# files are saved with `.mdx` extension


# helper methods to plot and execute flow in jupyter noteboopk
# Plot the flow
...
#########################
# `docs` dir will have generated documentation
flow = CreateDocumentationFlow()
flow.kickoff()


#########################
# plot one of the docs
#########################
